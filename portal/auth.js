...
const users = [
    {
        ...
        password: '$2b$10$DDN0DoYRR7G6R0Mj6YlemeBLRZ6juETwvBGWJ9o6hCw9cPJeP5igC',
        role: ROLES.ADMIN,
        name: 'Hunter Ligon'
    },
    {
...
const authenticateToken = (req, res, next) => {
    ...
    if (token) {
    jwt.verify(token, process.env.JWT_SECRET || 'portal-secret', (err, user) => {
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        // Dev-only fallback to ease testing locally
        const devMode = process.env.NODE_ENV !== 'production';
        const devPassword = process.env.DEV_PASSWORD || 'temporary';
        if (!validPassword && devMode && password === devPassword) {
            validPassword = true;
        }
        ...
        const token = jwt.sign({ 
            ...
            email: user.email,
            name: user.name
    }, process.env.JWT_SECRET || 'portal-secret', { expiresIn: '24h' });
        req.session.user = {
            id: user.id,
            username: user.username,
            role: user.role,
...
router.post('/create-client-access', authenticateToken, checkPermission('domains:create'), async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({
            ...
            domain: domain
    }, process.env.JWT_SECRET || 'portal-secret', { expiresIn: '30d' });
        res.json({
            success: true,
            clientAccess: {
                username: clientUser.username,
